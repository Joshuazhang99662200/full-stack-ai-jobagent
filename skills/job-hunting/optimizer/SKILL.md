...
改写视角由取回阶段观测到的招聘方身份决定,判定表与跨视角不变式见 [lens-routing.md](../references/optimizer/lens-routing.md)。

路由是本技能内置的判断力,**不可由外部插入**。来源(source)是可插拔的数据,视角不是:一条外部视角可以悄悄架空质量门禁。低置信时回退通用视角,不猜。
