...
第一条完整工作流必须能通过 `MockJobSource` 离线跑通。后续至少要有一个真实来源能够走完:搜索、抓取完整 JD、在可获取时取回招聘方信息、预览,并且只在拿到有效审批之后才提交。

搜索与抓取 JD 是**两项独立能力**,不必由同一个连接器承担:有的平台在结果页就给出完整 JD,有的只给列表字段、JD 另在详情页。后者用 `JobListingSource` + 独立的 JD 抓取补齐,绝不用列表字段合成 JD。
